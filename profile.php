...
$user_data_session = []; // Use a different variable name to avoid conflict with $user_data from DB
...
if (isLoggedIn()) {
    ...
    $user_data_session = $result_user->fetch_assoc();
    $stmt_user->close();
    $conn_local->close();
}

// Ensure $user_data_session has default values if not logged in or user_id invalid
if (!$user_data_session) {
    $user_data_session = ['nama' => 'Guest', 'foto_profil' => ''];
}
            ...
            if (isset($_FILES['foto_profil']) && $_FILES['foto_profil']['error'] == UPLOAD_ERR_OK) {
                ...
                if (!in_array($imageFileType, $allowed_types)) {
                    ...
                    if (!move_uploaded_file($_FILES['foto_profil']['tmp_name'], $target_file)) {
                        ...
                        if ($user_data['foto_profil'] && file_exists($target_dir . $user_data['foto_profil']) && $user_data['foto_profil'] !== 'default.png') { // Changed default.jpg to default.png
            ...
            if (empty($error)) { // Lanjutkan jika tidak ada error dari upload
                ...
                if ($email_check_result->num_rows > 0) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $_SESSION['foto_profil'] = $foto_profil_filename; // Update session foto_profil as well
            ...
            <div class="nav-user">
                ...
                <div class="dropdown profile-dropdown" id="profileDropdown">
                    <button class="profile-toggle">
                        <img src="<?php echo ($user_data_session['foto_profil'] && file_exists($upload_dir . $user_data_session['foto_profil'])) ? $upload_dir . htmlspecialchars($user_data_session['foto_profil']) : $upload_dir . 'default.png'; ?>" alt="Profile" class="profile-avatar">
                        <span class="profile-name"><?php echo htmlspecialchars($user_data_session['nama']); ?></span>
    ...
    <main>
        ...
        <div class="profile-container">
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                <input type="hidden" name="current_password_hash" value="<?php echo htmlspecialchars($user_data['password']); ?>">
